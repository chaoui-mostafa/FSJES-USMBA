...
<div class="py-6 px-4 sm:px-6 lg:px-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
    <div class="max-w-4xl mx-auto">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Détails de l'Utilisateur</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Profil complet et activités</p>
            </div>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('users.edit', $user->id) }}" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                    </svg>
                    Modifier
                </a>
                <a href="{{ route('users.index') }}" class="flex items-center gap-2 bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-md transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
        ...
        <!-- Messages de statut -->
        @if (session('success'))
            <div class="mb-6 p-4 bg-green-100 dark:bg-green-900 border-l-4 border-green-500 dark:border-green-400 text-green-700 dark:text-green-100 rounded">
                <p>{{ session('success') }}</p>
            </div>
        @endif
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
            ...
            <div class="px-4 py-5 sm:px-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100">Informations du Compte</h3>
            </div>
                ...
                <div class="flex flex-col sm:flex-row gap-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 flex-grow">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $user->telephone ?? 'Non renseigné' }}</dd>
                        </div>
                        <div>
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 px-4 py-5 sm:px-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100">Sécurité et Connexion</h3>
            </div>
            <div class="px-4 py-5 sm:p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 truncate" title="{{ $user->last_login_user_agent ?? '' }}">
            ...
            <!-- Section Actions -->
            <div class="border-t border-gray-200 dark:border-gray-700 px-4 py-5 sm:px-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100">Actions</h3>
            </div>
            <div class="px-4 py-4 sm:px-6 bg-gray-50 dark:bg-gray-700 flex justify-between flex-wrap gap-4">
                @if($user->isLocked)
                    <form action="{{ route('users.unlock', $user->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                ...
                <form action="{{ route('users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur ?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
